...
from .tokenizer_molglyph import get_molglyph_tokenizer
...
class MolGlyph:
    ...
    def __init__(self, model_path, device=None, num_workers=1):
        ...
        self.device = device
        self.tokenizer = get_molglyph_tokenizer(args)
        self.encoder, self.decoder = self._get_model(args, self.tokenizer, self.device, model_states)
        self.transform = build_transform(args.input_size)
    ...
    def _get_args(self, args_states=None):
        ...
        group.add_argument("--dec_num_layers", help="No. of layers in transformer decoder", type=int, default=12)
        group.add_argument("--dec_hidden_size", help="Decoder hidden size", type=int, default=512)
        group.add_argument("--dec_attn_heads", help="Decoder no. of attention heads", type=int, default=16)
    ...
    def predict_images(self, input_images: List, return_atoms_bonds=False, return_confidence=False, batch_size=16):
        ...
        for idx in range(0, len(input_images), batch_size):
            ...
            # input(batch_predictions)
            predictions += batch_predictions

        # input(predictions)
        smiles = [pred['molparser_caption']['smiles'] for pred in predictions]
